...
import net.sf.jasperreports.engine.{JasperCompileManager, JRDataSource, JasperFillManager}
  ...
  def print(entity:Entity) = {
    if(initEntity(entity)){
      ...
      createFieldMap(entity) match {
        case Some(map) => fieldDataSources :+= new JRMapArrayDataSource(Array(asMap(map)))
        case None =>
      }
